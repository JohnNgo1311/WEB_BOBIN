...
const empInputs = document.querySelectorAll(".input-inspector-code");

empInputs.forEach((empInput) => {
  empInput.addEventListener("input", function () {
    const val = this.value.toLowerCase().trim();
    const row = this.closest(".qc-info-row");

    if (!row) return;

    const empName = row.querySelector(".input-inspector-name");
    const empBox = this.closest(".qc-input").querySelector(".suggestion-box");
...
//TODO 6. CODE NHÂN VIÊN -> TÊN NHÂN VIÊN
document.addEventListener("DOMContentLoaded", function () {
  ...
  empInputs.forEach((empInput) => {
    empInput.addEventListener("input", function () {
      ...
      // ĐÃ SỬA DÒNG NÀY: Dùng parentElement thay vì closest('.qc-input') để tránh nhầm lẫn
